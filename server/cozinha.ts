...
export const lotesProducaoRouter = router({
  ...
  // Otimizado: filtra no banco e exclui itens que já estão no Kanban
  itensNecessarios: publicProcedure.query(async () => {
    const db = await getDb();
    if (!db) return [];

    // Buscar insumos da categoria Preparo com estoque baixo ou crítico
    // Identifica preparos por: Categoria "Preparo" OU código iniciando com "(PR)"
    const itens = await db.select()
      .from(insumos)
      .where(and(
        or(
          eq(insumos.categoria, 'Preparo'),
    ...
    // Buscar IDs dos insumos que já estão no Kanban (status diferente de 'finalizado')
    const lotesAtivos = await db.select({ insumoId: lotesProducao.insumoId })
      .from(lotesProducao)
      .where(sql`${lotesProducao.status} != 'finalizado'`);
    
    const idsNoKanban = new Set(lotesAtivos.map(l => l.insumoId));

    // Filtrar itens que já estão no Kanban para evitar duplicidade
    const itensFiltrados = itens.filter(item => !idsNoKanban.has(item.id));
