...
const dbPath = '/home/ubuntu/.manus/.webdev-state/painel-cozinha-sai/db/data.db';
...
// Atualizar fichas técnicas no banco
console.log('Atualizando fichas técnicas de Preparo...\n');
...
let updated = 0;
for (const [produto, modo] of Object.entries(modosPreparo)) {
  const result = updateStmt.run(modo, `%${produto}%`);
  if (result.changes > 0) {
...
const fichasAtualizadas = db.prepare(`
  SELECT id, produto, modoPreparo 
  FROM fichas_tecnicas 
  WHERE produto LIKE '%(PR)%' AND modoPreparo IS NOT NULL AND modoPreparo != ''
  ORDER BY produto
`).all();

console.log(`\nFichas de Preparo com modo de preparo: ${fichasAtualizadas.length}`);
